...
bool zero_mag = false;
double complex zero_meas[1000][NUM_CHANNELS];

extern "C" __declspec(dllexport) void fra_v8_2ch(struct sFRA_V8_2CH **opaque, double t, union uData *data)
{
   ...
   if(!*opaque)
   {
      *opaque = (struct sFRA_V8_2CH *) malloc(sizeof(struct sFRA_V8_2CH));
      bzero(*opaque, sizeof(struct sFRA_V8_2CH));

      struct sFRA_V8_2CH *inst = *opaque;
...
// Implement module evaluation code here:
   ...
   if(inst->status == 0)
   {
      ...
      if(inst->status == 1)
      {
         Display("\ti\tfreq\tsrc_mag[dB]\tsrc_arg[deg]\tin1_mag[dB]\tin1_arg[deg]\tin2_mag[dB]\tin2_arg[deg]\n");
   ...
   if(*t >= f->tsampling)
   {
      ...
      if(f->i > 0)
      {
         ...
         else
         {
            ...
            double complex zsrc = mag_p + I*0.0;
            double complex zmeas[NUM_CHANNELS];
            for(int ch = 0; ch < NUM_CHANNELS; ch++)
            {
               inam[ch] = 2/f->ts*(f->ina[ch]);
               inbm[ch] = 2/f->ts*(f->inb[ch]);

               zmeas[ch] = inam[ch] + I*inbm[ch];

               inmag[ch] = 20*log10(cabs(zmeas[ch]));
               inph[ch]  = 180/M_PI*carg(zmeas[ch]);
               ...
               if(f->ahi == 0 && f->alo == 0)
               {
                  zero_meas[(int)f->i][ch] = zmeas[ch];
                  zero_mag = true;
               }
            }

            Display("\t%d\t%f\t%f\t%f\t%f\t%f\t%f\n", (int)f->i, freq_p, 20*log10(cabs(zsrc)), 180/M_PI*carg(zsrc), inmag[0], inph[0], inmag[1], inph[1]);

            fptr = fopen(fname,"a");
            //fprintf(fptr,"%f\t%f,%f\t%f,%f\t%f,%f\n", freq_p, mag_p, 0.0, inam[0], inbm[0], inam[1], inbm[1]);
            ...
            if(f->ahi > 0 && f->alo > 0)
            {
               ...
               {
                  fprintf(fptr,"%f\t%f,%f\n", freq_p,
                     creal((zmeas[1] - zero_meas[(int)f->i][1])/(zmeas[0] - zero_meas[(int)f->i][0])),
                     cimag((zmeas[1] - zero_meas[(int)f->i][1])/(zmeas[0] - zero_meas[(int)f->i][0]))
                  );
               }
               else
               {
                  fprintf(fptr,"%f\t%f,%f\n", freq_p, creal(zmeas[1]/zmeas[0]), cimag(zmeas[1]/zmeas[0]));
               }
            }
            fclose(fptr);
         }
...
extern "C" __declspec(dllexport) double MaxExtStepSize(struct sFRA_V8_2CH *inst)
{
   return inst->maxstep; // implement a good choice of max timestep size that depends on struct sFRA_V3
}

extern "C" __declspec(dllexport) void Destroy(struct sFRA_V8_2CH *inst)
{
