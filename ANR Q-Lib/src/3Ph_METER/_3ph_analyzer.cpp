...
extern "C" __declspec(dllexport) void _3ph_analyzer(struct sDATA **opaque, double t, union uData *data)
{
   ...
   double fund  = data[7].d; // input parameter
   double vnom  = data[8].d; // input parameter
...
extern "C" __declspec(dllexport) void Destroy(struct sDATA *inst)
{
   ...
   {
      Display("\n\n");
      Display("Ptotal\tQtotal\tStotal\tPFavg\t");
      Display("Vrmsavg\tIrmsavg\tVthdavg\tIthdavg\t");
      Display("P1\tQ1\tPF1\tvfund1\tvrms1\tvthd1\tifund1\tirms1\tithd1\t");
      Display("P2\tQ2\tPF2\tvfund2\tvrms2\tvthd2\tifund2\tirms2\tithd2\t");
      Display("P3\tQ3\tPF3\tvfund3\tvrms3\tvthd3\tifund3\tirms3\tithd3\n");

      fptr = fopen("3ph_analyzer.csv","w");
      fprintf(fptr,"\n\n");
      fprintf(fptr,"Ptotal,Qtotal,Stotal,PFavg,Vrmsavg,Irmsavg,Vthdavg,Ithdavg,");
   ...
   Display("%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\n",
      p1 + p2 + p3, q1 + q2 + q3, vrms1 * irms1 + vrms2 * irms2 + vrms3 * irms3, (pf1 + pf2 + pf3) / 3,
      (vrms1 + vrms2 + vrms3) / 3, (irms1 + irms2 + irms3) / 3, (vthd1 + vthd2 + vthd3) / 3 * 100., (ithd1 + ithd2 + ithd3) / 3 * 100.,
   ...
   fptr = fopen("3ph_analyzer.csv","a");
   fprintf(fptr,"%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f\n",
      p1 + p2 + p3, q1 + q2 + q3, vrms1 * irms1 + vrms2 * irms2 + vrms3 * irms3, (pf1 + pf2 + pf3) / 3,
      (vrms1 + vrms2 + vrms3) / 3, (irms1 + irms2 + irms3) / 3, (vthd1 + vthd2 + vthd3) / 3 * 100., (ithd1 + ithd2 + ithd3) / 3 * 100.,
      p1, q1, pf1, vfund1, vrms1, vthd1 * 100, ifund1, irms1, ithd1 * 100,
      p2, q2, pf2, vfund2, vrms2, vthd2 * 100, ifund2, irms2, ithd2 * 100,
