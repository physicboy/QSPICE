...
extern "C" __declspec(dllexport) void sine_c(struct sSINE_C **opaque, double t, union uData *data)
{
   ...
   if(!*opaque)
   {
      *opaque = (struct sSINE_C *) calloc(1, sizeof(struct sSINE_C));
      (*opaque)->ConnectSocket = BerkeleySocket(server, Ninputs, Noutputs, &(*opaque)->buffer);
   }
   struct sSINE_C *inst = *opaque;
...
extern "C" __declspec(dllexport) double MaxExtStepSize(struct sSINE_C *inst, double t)
{
   return inst->max_step;
}

extern "C" __declspec(dllexport) void Destroy(struct sSINE_C *inst)
{
