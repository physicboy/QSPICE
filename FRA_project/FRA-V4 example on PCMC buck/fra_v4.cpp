...
//
//    dmc -mn -WD fra_v4.cpp kernel32.lib
...
extern "C" __declspec(dllexport) void fra_v4(struct sFRA_V4 **opaque, double t, union uData *data)
{
   ...
   if(!*opaque)
   {
      *opaque = (struct sFRA_V4 *) malloc(sizeof(struct sFRA_V4));
      bzero(*opaque, sizeof(struct sFRA_V4));

      struct sFRA_V4 *inst = *opaque;
...
extern "C" __declspec(dllexport) double MaxExtStepSize(struct sFRA_V4 *inst)
{
   return inst->maxstep; // implement a good choice of max timestep size that depends on struct sFRA_V3
}

extern "C" __declspec(dllexport) void Destroy(struct sFRA_V4 *inst)
{
