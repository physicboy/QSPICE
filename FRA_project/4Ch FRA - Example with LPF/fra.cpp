...
extern "C" __declspec(dllexport) void fra(struct sFRA **opaque, double t, union uData *data)
{
   ...
   double  ina3               = data[ 8].d; // input
   double  inb3               = data[ 9].d; // input
   double  ina4               = data[10].d; // input
   double  inb4               = data[11].d; // input
   ...
   double  f_min              = data[16].d; // input parameter
   double  f_max              = data[17].d; // input parameter
   double  lin0_log1          = data[18].d; // input parameter
   double  f_step             = data[19].d; // input parameter
   double  tstep_factor       = data[20].d; // input parameter
   double  pre_dwell          = data[21].d; // input parameter
   double  sine_dwell_mintime = data[22].d; // input parameter
   double  sine_dwell_period  = data[23].d; // input parameter
   double  sine_meas_mintime  = data[24].d; // input parameter
   double  ttol               = data[25].d; // input parameter
   double  sine_meas_period   = data[26].d; // input parameter
   double &status             = data[27].d; // output
   double &amp                = data[28].d; // output
   double &freq               = data[29].d; // output
   ...
   if((inst->t_prev <= inst->t_step)&&(t >= inst->t_step))
   {
      if(inst->cntr < 0)
      {
        ...
        if(cntr_print<inst->cntr)
        {
          ...
          display("param3:\t%f\n",param3);
          display("param4:\t%f\n",param4);
          display("No\tFreq\tMag[IN1](dB)\tPh[IN1](deg)\tMag[IN2](dB)\tPh[IN2](deg)\tMag[IN3](dB)\tPh[IN3](deg)\tMag[IN4](dB)\tPh[IN4](deg)\telapsed time\n");
          #endif
          ...
          fprintf(fptr,"No\tFreq\tMag[IN1](dB)\tPh[IN1](deg)\tMag[IN2](dB)\tPh[IN2](deg)\tMag[IN3](dB)\tPh[IN3](deg)\tMag[IN4](dB)\tPh[IN4](deg)\telapsed time\n");
          fclose(fptr);
          cntr_print = inst->cntr;
        }
      }
      else
      {
       ...
       double ph_out1_prev = inst->ph_out1;
       double ph_out2_prev = inst->ph_out2;
       double ph_out3_prev = inst->ph_out3;
       double ph_out4_prev = inst->ph_out4;
         ...
         inst->ina3_interp = inst->ina3_prev + (ina3 - inst->ina3_prev)/(t - inst->t_prev)*(inst->t_step - inst->t_prev);
         inst->inb3_interp = inst->inb3_prev + (inb3 - inst->inb3_prev)/(t - inst->t_prev)*(inst->t_step - inst->t_prev);
         inst->ina4_interp = inst->ina4_prev + (ina4 - inst->ina4_prev)/(t - inst->t_prev)*(inst->t_step - inst->t_prev);
         inst->inb4_interp = inst->inb4_prev + (inb4 - inst->inb4_prev)/(t - inst->t_prev)*(inst->t_step - inst->t_prev);
         ...
         inst->a3 = 2*(inst->ina3_interp - inst->ina3_sample)/(inst->t_step - inst->t_sample);
         inst->b3 = 2*(inst->inb3_interp - inst->inb3_sample)/(inst->t_step - inst->t_sample);
         inst->mag_out3 = sqrt(inst->a3*inst->a3 + inst->b3*inst->b3);
         inst->ph_out3 = -1*atan2(inst->b3,inst->a3)*180/M_PI;

         inst->a4 = 2*(inst->ina4_interp - inst->ina4_sample)/(inst->t_step - inst->t_sample);
         inst->b4 = 2*(inst->inb4_interp - inst->inb4_sample)/(inst->t_step - inst->t_sample);
         inst->mag_out4 = sqrt(inst->a4*inst->a4 + inst->b4*inst->b4);
         inst->ph_out4 = -1*atan2(inst->b4,inst->a4)*180/M_PI;

       inst->ph_out1 += inst->ph1_unwrap;
       inst->ph_out2 += inst->ph2_unwrap;
       inst->ph_out3 += inst->ph3_unwrap;
       inst->ph_out4 += inst->ph4_unwrap;

       if(inst->ph_out1 > (ph_out1_prev + 270))
       {
          inst->ph1_unwrap -= 360;
          inst->ph_out1 -= 360;
       }
       ...
       if(inst->ph_out2 > (ph_out2_prev + 270))
       {
          inst->ph2_unwrap -= 360;
          inst->ph_out2 -= 360;
       }
       ...
       if(inst->ph_out3 > (ph_out3_prev + 270))
       {
          inst->ph3_unwrap -= 360;
          inst->ph_out3 -= 360;
       }
       ...
       if(inst->ph_out4 > (ph_out4_prev + 270))
       {
          inst->ph4_unwrap -= 360;
          inst->ph_out4 -= 360;
       }
         ...
         if(cntr_print<inst->cntr)
         {
            if(inst->cntr > 0)
            {
               if(inst->cntr <= f_step)
               {
                  ...
                  #ifdef __DISPLAY
                  display("%i\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%.3f\n",
                     (int)inst->cntr, inst->fstep_now,
                     20*log10(inst->mag_out1), inst->ph_out1,
                     20*log10(inst->mag_out2), inst->ph_out2,
                     20*log10(inst->mag_out3), inst->ph_out3,
                     20*log10(inst->mag_out4), inst->ph_out4,
                     ((float)(clock() - t_begin)) / CLOCKS_PER_SEC);
                  #endif

                  fptr = fopen(fname,"a");
                  fprintf(fptr,"%i\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%05.5f\t%.3f\n",
                     (int)inst->cntr, inst->fstep_now,
                     20*log10(inst->mag_out1), inst->ph_out1,
                     20*log10(inst->mag_out2), inst->ph_out2,
                     20*log10(inst->mag_out3), inst->ph_out3,
   ...
   if((inst->t_prev <= inst->t_sample)&&(t >= inst->t_sample))
   {
      ...
      inst->ina3_sample = inst->ina3_prev + (ina3 - inst->ina3_prev)/(t - inst->t_prev)*(inst->t_sample - inst->t_prev);
      inst->inb3_sample = inst->inb3_prev + (inb3 - inst->inb3_prev)/(t - inst->t_prev)*(inst->t_sample - inst->t_prev);
      inst->ina4_sample = inst->ina4_prev + (ina4 - inst->ina4_prev)/(t - inst->t_prev)*(inst->t_sample - inst->t_prev);
      inst->inb4_sample = inst->inb4_prev + (inb4 - inst->inb4_prev)/(t - inst->t_prev)*(inst->t_sample - inst->t_prev);
   }
