...
// Implement module evaluation code here:
   ...
   if(inst->pwm!=_pwm)
   {
      inst->trgdelay = t + dtime/mcu_clk;
   }

   if((inst->t_prev <= inst->trgdelay)&&(t >= inst->trgdelay))
   {
      inst->xcntr++;
      inst->pwm_delay = inst->pwm;
      inst->pwm_trigger = true;
   }

   pwmhi = inst->pwm*inst->pwm_delay;
   pwmlo = (1 - inst->pwm)*(1 - inst->pwm_delay);
...
{ // limit the timestep to a tolerance if the circuit causes a change in struct sEPWM
   ...
   if(inst->t_prev < inst->trgdelay){if(*timestep > (inst->trgdelay - inst->t_prev)){*timestep = (inst->trgdelay - inst->t_prev);}}
   if(inst->pwm_trigger)
   {
