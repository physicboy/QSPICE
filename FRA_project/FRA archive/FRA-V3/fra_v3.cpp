...
//
//    dmc -mn -WD fra_v3.cpp kernel32.lib
...
void FRA_CORE(struct fra_data *f, double *t,
      double *in1a, double *in1b, double *in2a, double *in2b);

FILE *fptr;
char *fname = "fra.csv";

extern "C" __declspec(dllexport) void fra_v3(struct sFRA_V3 **opaque, double t, union uData *data)
{
   double  IN1A          = data[ 0].d; // input
   double  IN1B          = data[ 1].d; // input
   double  IN2A          = data[ 2].d; // input
   double  IN2B          = data[ 3].d; // input
   double  param1        = data[ 4].d; // input
   double  param2        = data[ 5].d; // input
   double  param3        = data[ 6].d; // input
   double  param4        = data[ 7].d; // input
   double  f_min         = data[ 8].d; // input parameter
   double  f_max         = data[ 9].d; // input parameter
   double  a_lo          = data[10].d; // input parameter
   double  a_hi          = data[11].d; // input parameter
   double  f_lo          = data[12].d; // input parameter
   double  f_hi          = data[13].d; // input parameter
   bool    lin0_log1     = data[14].b; // input parameter
   double  tstep_factor  = data[15].d; // input parameter
   double  ss_tmax       = data[16].d; // input parameter
   double  dwell_mintime = data[17].d; // input parameter
   double  dwell_period  = data[18].d; // input parameter
   double  meas_mintime  = data[19].d; // input parameter
   double  meas_period   = data[20].d; // input parameter
   int     f_step        = data[21].i; // input parameter
   bool    plot          = data[22].b; // input parameter
   double &amp           = data[23].d; // output
   double &freq          = data[24].d; // output

   if(!*opaque)
   {
      *opaque = (struct sFRA_V3 *) malloc(sizeof(struct sFRA_V3));
      bzero(*opaque, sizeof(struct sFRA_V3));

      struct sFRA_V3 *inst = *opaque;
...
// Implement module evaluation code here:
   ...
   {
      FRA_CORE(&inst->fra, &t, &IN1A, &IN1B, &IN2A, &IN2B);
...
void FRA_CORE(struct fra_data *f, double *t, double *in1a, double *in1b, double *in2a, double *in2b)
{
   if(*t >= f->tsampling)
   {
      f->i = f->i + 0.5;
...
extern "C" __declspec(dllexport) double MaxExtStepSize(struct sFRA_V3 *inst)
{
   return inst->maxstep; // implement a good choice of max timestep size that depends on struct sFRA_V3
}

extern "C" __declspec(dllexport) void Destroy(struct sFRA_V3 *inst)
{
