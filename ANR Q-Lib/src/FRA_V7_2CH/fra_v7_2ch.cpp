...
extern "C" __declspec(dllexport) void fra_v7_2ch(struct sFRA_V7_2CH **opaque, double t, union uData *data)
{
   ...
   if(!*opaque)
   {
      *opaque = (struct sFRA_V7_2CH *) malloc(sizeof(struct sFRA_V7_2CH));
      bzero(*opaque, sizeof(struct sFRA_V7_2CH));

      struct sFRA_V7_2CH *inst = *opaque;
...
extern "C" __declspec(dllexport) double MaxExtStepSize(struct sFRA_V7_2CH *inst)
{
   return inst->maxstep; // implement a good choice of max timestep size that depends on struct sFRA_V3
}

extern "C" __declspec(dllexport) void Destroy(struct sFRA_V7_2CH *inst)
{
